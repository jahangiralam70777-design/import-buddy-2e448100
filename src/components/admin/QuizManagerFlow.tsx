...
export function QuizManagerFlow() {
  ...
  const kpis = [
    { l: "Total Quizzes", v: stats.data?.total ?? 0, d: stats.data?.deltas.total ?? 0, i: ListChecks, c: "#a855f7" },
    { l: "Published", v: stats.data?.published ?? 0, d: stats.data?.deltas.published ?? 0, i: Send, c: "#22d3ee" },
    { l: "Drafts", v: stats.data?.draft ?? 0, d: stats.data?.deltas.draft ?? 0, i: Edit3, c: "#fbbf24" },
    { l: "Scheduled", v: stats.data?.scheduled ?? 0, d: 0, i: CalendarClock, c: "#60a5fa" },
    { l: "Archived", v: stats.data?.archived ?? 0, d: 0, i: Archive, c: "#94a3b8" },
    { l: "Total Attempts", v: stats.data?.attempts ?? 0, d: stats.data?.deltas.attempts ?? 0, i: Trophy, c: "#34d399", fmt: "k" as const },
  ];
  const kpis2 = [
    { l: "Completion Rate", v: stats.data?.completionRate ?? 0, d: 0, i: PercentSquare, c: "#10b981", fmt: "pct" as const },
    { l: "Avg. Score", v: stats.data?.avgScore ?? 0, d: 0, i: Target, c: "#f472b6", fmt: "pct" as const },
    { l: "Active Users (24h)", v: stats.data?.activeUsers ?? 0, d: 0, i: Users, c: "#38bdf8" },
    { l: "Performance Score", v: stats.data?.performanceScore ?? 0, d: 0, i: Gauge, c: "#f59e0b", fmt: "pct" as const },
    { l: "AI Generated", v: stats.data?.aiGenerated ?? 0, d: stats.data?.deltas.aiGenerated ?? 0, i: Bot, c: "#a78bfa" },
  ];
  ...
  return (
    <div className="space-y-4 p-4 lg:p-6">
      ...
      <section className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
        {kpis.map((k) => <KpiCard key={k.l} {...k} />)}
      </section>
      {/* KPI cards — secondary row */}
      <section className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-5">
        {kpis2.map((k) => <KpiCard key={k.l} {...k} />)}
      </section>
...
function KpiCard({ l, v, d, i: Icon, c, fmt }: KpiProps) {
  const up = d >= 0;
  const fmtVal =
    fmt === "k" && v >= 1000 ? `${(v / 1000).toFixed(1)}K`
      ...
      : v.toLocaleString();
  return (
    <div className="group glass shadow-card-soft rounded-2xl border border-border/60 p-4 transition hover:-translate-y-0.5 hover:shadow-xl">
